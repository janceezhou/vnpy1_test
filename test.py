...
class TurtleStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        # 创建K线合成器对象
        self.bg = BarGenerator(self.onBar, 60, self.onXminBar) 
    ...
    def onTick(self, tick):
        ...
        if len(self.orderList) == 0:
            self.unit = int(self.totalMoney / self.atrValue)   
            ...
            if not (self.lastBreakLongTrade or self.lastBreakShortTrade):
                ...
                if tick.lastPrice < self.historicLow20:
                    self.lastBreakOutPrice = tick.lastPrice
                    self.lastBreakAtrValue = self.atrValue
                ...
                if tick.lastPrice > self.historicHigh10:
                    self.lastBreakLongTrade = False
                ...
                if tick.lastPrice > self.lastBreakOutPrice + 2 * self.lastBreakAtrValue:                
                    ...
                    self.lastBreakLosing = True
                if tick.lastPrice < self.historicLow10:
                    self.lastBreakShortTrade = False
                    self.lastBreakLosing = False
                    
        # 持有多头仓位
        elif self.pos > 0:
            if tick.lastPrice > self.lastTradePrice + len(orderList) * self.lastTradeAtrValue / 2:
                self.orderList.extend(self.buy(tick.lastPrice, self.unit, False))
            
            self.longStop = self.lastTradePrice - (2 + (len(orderList) - 1) / 2) * self.lastTradeAtrValue
            
            if tick.lastPrice < self.longStop:
        ...
        # 持有空头仓位
            ...
            self.shortStop = self.lastTradePrice + (2 - (len(orderList) - 1) / 2) * self.lastTradeAtrValue
            
            if tick.lastPrice > self.shortStop:
